...
# blog: https://zhuanlan.zhihu.com/p/538486008
import torch
...
if Switch == 0:
    ...
    for episode in range(EP_MAX):
        ...
        if episode % 50 == 0 and episode > 100:  # 保存神经网络参数
            ...
            torch.save(save_data, "../outputs/weights/PPO2_model_actor.pth")
            save_data = {'net': critic.critic_v.state_dict(), 'opt': critic.optimizer.state_dict(), 'i': episode}
            torch.save(save_data, "../outputs/weights/PPO2_model_critic.pth")
    env.close()
    ...
    aa = Actor()
    cc = Critic()
    checkpoint_aa = torch.load("../outputs/weights/PPO2_model_actor.pth")
    aa.old_pi.load_state_dict(checkpoint_aa['net'])
    checkpoint_cc = torch.load("../outputs/weights/PPO2_model_critic.pth")
