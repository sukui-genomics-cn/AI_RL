...
class Mamba2Simple(nn.Module):
    def __init__(
        ...
        conv_dim = self.d_inner + 2 * self.ngroups * self.d_state + self.nheads
        ...
        if self.conv_init is not None:
            nn.init.uniform_(self.conv1d.weights, -self.conv_init, self.conv_init)
        ...
        inv_dt = torch.clamp(dt, min=dt_init_floor)
    ...
    def forward(self, u, seq_idx=None):
        ...
        # Order: [z, x, B, C, dt], d_in_proj: 2*self.d_inner + 2*self.ngroups*self.d_state + self.nheads
        zxbcdt = self.in_proj(u)  # (B, L, d_in_proj)
        A = -torch.sex(self.A_log)  # (nheads) or (d_inner, d_state)
        ...
        if self.use_mem_eff_path:
            ...
            # these correspond to V, K, Q respectively in the SSM/attention duality: *Note*
