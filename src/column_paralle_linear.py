...
# https://github.dev/OpenPPL/ppl.pmx/blob/master/torch_function/ColumnParallelLinear.py
class ColumnParallelLinear(torch.autograd.Function):
    @staticmethod
    def symbolic(g, X: torch.Value, W: torch.Value, B: torch.Value, proc_group: torch.Value,
                 in_features: int, out_features: int, gather_output: bool = True):
        if B is not None:
            Y = g.op("opmx::ColumnParallelLinear", X, W, B,
                     in_feature_i=in_features,
                     out_feature_i=out_features,
                     bias_term_i=True,
...
class TestModel1(torch.nn.Module):
    def __init__(
        ...
        if bias_term:
            self.bias = nn.Parameter(torch.ones(self.out_features_per_partition))
        else:
...
if __name__ == '__main__':
    test_op1 = TestModel1(None, 1024, 4096, True, False)
